...
class PydanticSourceModelConverter[S: pd.BaseModel, T](abc.ABC):
    ...
    @classmethod
    def convert_to_model_and_dict(cls, source_model: S) -> tuple[T, dict[str, t.Any]]:
        """
        ...
        cls._pre_convert_model(target_model_data)
        ...
        for field_name in source_model.model_fields:
            ...
            cls._execute_field_converter(cls._pre_convert_field, field_data, target_model_data)

            if not field_data.is_converted:
                if isinstance(value, cls.get_source_model_base_class()):
                    model, dict_ = cls.convert_to_model_and_dict(value)
                    ...
                    for item in value:
                        if isinstance(item, cls.get_source_model_base_class()):
                            model, dict_ = cls.convert_to_model_and_dict(item)
            ...
            if not field_data.is_skip_post_convert:
                cls._execute_field_converter(cls._post_convert_field, field_data, target_model_data)

            if not field_data.is_converted:
                target_model_data.update(field_name, value)

        cls._post_convert_model(target_model_data)
    ...
    @classmethod
    def _pre_convert_model(cls, model_data: TargetModelData) -> None:
        pass

    @classmethod
    def _pre_convert_field(cls, field_data: FieldData, model_data: TargetModelData) -> bool:
        return False
    
    @classmethod
    def _post_convert_field(cls, field_data: FieldData, model_data: TargetModelData) -> bool:
        return False
    
    @classmethod
    def _post_convert_model(cls, model_data: TargetModelData) -> None:
        pass
