...
class ServiceAdapter[H, L]:
    ...
    def create(self, higher_model: H) -> int:
        lower_model = self.model_converter.convert_to_lower_model(higher_model)
        return self.adapted_service.create(lower_model)

    def update(self, higher_model: H) -> None:
        lower_model = self.model_converter.convert_to_lower_model(higher_model)
        self.adapted_service.update(lower_model)

    def delete(self, higher_model_class: type[H], pk: int) -> None:
        lower_model_class = self.model_converter.get_lower_model_class(higher_model_class)
        self.adapted_service.delete(lower_model_class, pk)
