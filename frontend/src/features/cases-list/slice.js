...
export const getCasesList = createAsyncThunk('casesList/getCasesList', () => {
        ...
        builder.addCase(getCasesList.fulfilled, (state, action) => {
            ...
            for (let i = 0; i < data.length; ++i) {
                ...
                if (value['serious']) {
                    ...
                    value['serious'] = 'Non serious'
                }

                if (Array.isArray(value['drug_names'])) {
                    value['drug_names'] = parseArrayData(value['drug_names']);
                }
                if (Array.isArray(value['reaction_names'])) {
                    value['reaction_names'] = parseArrayData(value['reaction_names']);
                }
