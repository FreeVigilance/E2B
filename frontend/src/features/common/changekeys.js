...
function changeKeysFactory(changeCase) {
    return function changeKeys(object, depth = -1, options) {
        ...
        Object.keys(object).forEach((key) => {
            ...
            if (
                changedKey.indexOf('id') >= 0 &&
                changedKey !== 'id' &&
                changedKey.indexOf('MiddleName') < 0 &&
                changedKey.indexOf('Identification') < 0
            ) {
...
function pascalSnakeCase(input, options) {
    ...
    return (
        changeCase.constantCase(input.substr(0, 1)) +
        input.substr(1, prefixEnd - 2) +
        '_' +
        changeCase.capitalCase(input.substr(prefixEnd), { delimiter: '' })
    );
}
