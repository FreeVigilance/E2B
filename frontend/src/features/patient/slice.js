...
export const getPatient = () => {
    return (dispatch, getState) => {
        const patientData = getState().patient.patientData;
        const medicalHistory = getState().patient.medicalHistory;
        const drugHistory = getState().patient.drugHistory;
        const autopsy = getState().patient.autopsy;
        const causeOfDeath = getState().patient.causeOfDeath;
        const parentChildData = getState().patient.parentChildData;
        const parentHistoryData = getState().patient.parentHistoryData;
        const parentData = getState().patient.parentData;
        const parentDrugHistory = getState().patient.parentDrugHistory;
...
export const parsePatient = (jsonData) => {
    return (dispatch, getState) => {
        const data = jsonData.D_PatientCharacteristics;
        ...
        Object.values(data.D_8_r_PastDrugHistory).forEach((item, index) => {
            const itemData = new DrugHistory();
        ...
        Object.values(data.D_9_CaseDeath.D_9_2_r_CauseDeath).forEach((item, index) => {
            const itemData = new CauseOfDeath();
        ...
        Object.values(data.D_9_CaseDeath.D_9_4_r_AutopsyDeterminedCauseDeath).forEach((item, index) => {
            const itemData = new AutopsyData();
        ...
        const parentChildData = new ParentChildData();
        const parentChildJson = data.D_10_InformationConcerningParent;
        ...
        const parentHistoryData = new ParentHistoryData();
